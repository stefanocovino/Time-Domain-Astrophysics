using ColorTypes, FFTW, StatsFuns, Statistics, SpecialFunctions
...
# Minor update by Stefano Covino (stefano.covino@inaf.it)
#
...
#   1D continous Wavelet transform
#
...
function wavelet_tc(y,dt,pad=0,dj=-1,s0=-1,j1=-1,mother=-1,param=-1)

    n1 = length(y)
    if s0 == -1; s0=2*dt; end
    if dj == -1; dj = 1. / 4.; end
    if j1 == -1
        j1=Int(trunc(Int,(log(n1*dt/s0)/log(2))/dj))
    else 
        j1 = Int(j1)
    end
    if mother == -1; mother = "MORLET"; end
