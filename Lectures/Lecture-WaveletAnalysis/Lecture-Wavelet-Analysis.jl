...
# ╔═╡ b1be49de-f752-4fb2-b662-71ba299831c2
WidthOverDocs()
...
md"""
Wavelet central frequency $ω_0$: $( @bind ω0_obs PlutoUI.Slider(1.:1.:20., default=1.) ) 
"""
...
begin
	...
	real_part = real.(morlet(time_range, ω0_obs))
	imag_part = imag.(morlet(time_range, ω0_obs))
	envelope  = abs.(morlet(time_range, ω0_obs))
	
	figmor = Figure(size = (800, 600))
	axmor  = Axis(figmor[1, 1], 
	           title = "Morlet Wavelet (ω₀ = $(round(ω0_obs, digits=1)))",
...
# ╔═╡ c5bbf83a-8ff7-417e-853e-2418d656e954
md"""

#### Let's see a wavelet "in action":

- We show step by step how a *Morlet wavelet* slides along a signal and produces the *convolution coefficients* (continuous wavelet transform).

"""

# ╔═╡ a1a2794c-65cc-4117-b826-f97c22bd3a86
md"Wavelet position τ (s):"

# ╔═╡ 500aa07a-2019-4147-b4e6-ad09466df169
@bind τ_idx PlutoUI.Slider(1:200; default=50, show_value=false)

# ╔═╡ eaaae994-c24b-415d-9f22-7e54ae0c285b
md"Central frequency f₀ (Hz):"

# ╔═╡ e8c99b47-c942-425c-8acf-1d81b153f025
@bind f0 PlutoUI.Slider(4:2:30; default=8, show_value=true)

# ╔═╡ 8b545f05-71d9-4fdf-8e7d-9fa1d3629378
md"Width σ of the Gaussian envelope (time window):"

# ╔═╡ 760eecbd-4a1c-46a3-8d19-8a17f203aa5a
@bind σ PlutoUI.Slider(0.01:0.005:0.12; default=0.04, show_value=true)

# ╔═╡ 3f24bdd2-ece3-4d91-8e24-95bbe3ad61f5
begin
    const Np    = 512
    const t_ax = collect(range(0.0, 1.0, length=Np))
    const τ_ax = collect(range(0.02, 0.98, length=200))
 
    function make_signal(t)
        s1 = @. 0.4 * sin(2π * 6  * t) * exp(-((t - 0.25)^2) / 0.003)
        s2 = @. 0.7 * sin(2π * 20 * t) * exp(-((t - 0.72)^2) / 0.002)
        s1 .+ s2
    end
 
    const sig = make_signal(t_ax)
 
    function morlet_wave(t, τ, f0, σ)
        @. exp(-((t - τ)^2) / (2σ^2)) * cos(2π * f0 * (t - τ))
    end
 
    conv_coeff(s, w) = sum(s .* w) / length(s)
end;

# ╔═╡ 2c4afbbe-ffe7-479d-959f-ade4ce26a2a6
begin
    τ_current = τ_ax[τ_idx]
    wav       = morlet_wave(t_ax, τ_current, f0, σ)
    prod_sig  = sig .* wav
 
    conv_τ   = τ_ax[1:τ_idx]
    conv_val = [conv_coeff(sig, morlet_wave(t_ax, τ_ax[k], f0, σ)) for k in 1:τ_idx]
    c_current = conv_coeff(sig, wav)
end;
...
let
   ...
   #     fontsize=13, color=:gray40)
    ...
    ax1 = Axis(fig[1,1]; title="s(t) — original signal",
               ...
               titlesize=13, ylabelsize=11)
 
    ax2 = Axis(fig[2,1]; title="ψ(t−τ) — Morlet wavelet",
               ylabel="amplitude", xticklabelsvisible=false,
               titlesize=13, ylabelsize=11)
 
    ax3 = Axis(fig[3,1]; title="s(t)·ψ(t−τ) — pointwise product",
               ylabel="product", xticklabelsvisible=false,
               titlesize=13, ylabelsize=11)
 
    ax4 = Axis(fig[4,1]; title="W(τ) — accumulated convolution",
               xlabel="τ [s]", ylabel="coeff.",
               titlesize=13, xlabelsize=11, ylabelsize=11)
 
    rowgap!(fig.layout, 6)
 
    # ── Panel 1: signal
    lines!(ax1, t_ax, sig, color=:steelblue, linewidth=1.6)
    vlines!(ax1, [τ_current], color=(:darkorange, 0.7),
            linewidth=1.4, linestyle=:dash)
    ylims!(ax1, -1.3, 1.3)
 
    # ── Panel 2: wavelet + Gaussian envelope
    env = @. exp(-((t_ax - τ_current)^2) / (2σ^2))
    band!(ax2, t_ax, .-env, env, color=(:darkorange, 0.10))
    lines!(ax2, t_ax, wav, color=:darkorange, linewidth=1.6)
    vlines!(ax2, [τ_current], color=(:darkorange, 0.8),
            linewidth=1.4, linestyle=:dash)
    ylims!(ax2, -1.3, 1.3)
 
    # ── Panel 3: pointwise product
    band!(ax3, t_ax,
          min.(prod_sig, 0.0),
          max.(prod_sig, 0.0),
          color=(:mediumpurple, 0.30))
    lines!(ax3, t_ax, prod_sig, color=:mediumpurple, linewidth=1.4)
    hlines!(ax3, [0.0], color=(:black, 0.20), linewidth=0.7)
    text!(ax3, τ_current, 0.48;
          text="∫ = $(round(c_current, digits=4))",
          fontsize=11, color=:mediumpurple, align=(:center, :center))
    ylims!(ax3, -0.65, 0.65)
 
    # ── Panel 4: partial convolution
    lines!(ax4, conv_τ, conv_val, color=:crimson, linewidth=2.0)
    scatter!(ax4, [τ_current], [c_current],
...
md"""
- When the wavelet overlaps a burst at the same frequency as $f₀$, the pointwise product is consistently positive.
- Increasing $f₀$ from 8 to 20 Hz we see the first burst (6 Hz) disappearing, while the second emerges.
- Decreasing $σ$ we have a narrower window that gives better time resolution but poorer frequency resolution.
"""
...
# ╔═╡ bf986e7a-3380-4efe-afbb-c8d9f1654139
md"Notebook v1.1.0 - 9 September 2026"
...
manifest_format = "2.0"
project_hash = "8f702bde7088775c2371125b6714c1f201cc0f3b"
