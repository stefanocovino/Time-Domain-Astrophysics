...
# ╔═╡ e0c7f11b-f541-49d3-af4e-8f1744ddbe95
WidthOverDocs()
...
cm"- Let's check the results by a simple MonteCarlo approach, generating a large set of uniformly distributed time of arrivals"

# ╔═╡ e4abe6c6-02a3-454b-b72a-d34e5b660252
begin
	resb(x) = maximum(z2n(freq,x))
	bres = []
	for i in 1:10000
		artdt = rand(minimum(DOevts):maximum(DOevts),length(DOevts))
		push!(bres,resb(artdt))
	end
end

# ╔═╡ 71479d0d-ca9a-4e3b-a1b9-2b4a5c14a7d0
begin
	fbt = Figure()
...
# ╔═╡ f39275b4-1edb-443e-a048-6c7311e9c8b2
pr = 100 * count(<=(pmax), bres) / length(bres);

# ╔═╡ 3cebc5ae-07f6-403f-b065-8006c6cefd55
cm"""
...
md"Notebook v1.2.0 - 12 September 2026"
...
manifest_format = "2.1"
project_hash = "a197f9d1a5bb1729246271d0a6d2dd269f1d447f"
