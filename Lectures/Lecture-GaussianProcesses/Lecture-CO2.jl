...
# ╔═╡ 421905d1-d5b0-4b0f-91f1-3ce0e6318944
WidthOverDocs()
...
begin
	function plotdata()
	    ...
	    scatter!(xtrain, ytrain; label="training data", markersize=5, strokewidth=0,color=:blue)
	    scatter!(xtest, ytest; label="test data", markersize=5, strokewidth=0, color=:orange)
...
cm"- Then, let's begin to build the GP priors (our kernel):"

# ╔═╡ d0610eac-e675-4937-a3cd-f087f3ee5ad5
begin
	SE(θ) = θ.σ^2 * with_lengthscale(SqExponentialKernel(), θ.ℓ)
	Per(θ) = with_lengthscale(PeriodicKernel(; r=[θ.ℓ / 2]), θ.p)  
	RQ(θ) = θ.σ^2 * with_lengthscale(RationalQuadraticKernel(; α=θ.α), θ.ℓ);
end

# ╔═╡ 2b6d2d87-5954-4d5b-8806-42ef138613e4
cm"- This allows us to write a function that, given the nested tuple of parameter values, constructs the GP prior:"

# ╔═╡ 1ed555e5-d471-4a16-9bd2-e41771772cb5
...
# ╔═╡ d7bc6ec3-7c4c-41b6-924d-307454606132
cm"- To construct the posterior, we need to first build a FiniteGP, which represents the infinite-dimensional GP at a finite number of input features:"

# ╔═╡ 9d0b62b3-536c-4bb4-a50d-a194b6160fdc
function build_finite_gp(θ)
    f = build_gp_prior(θ)
    return f(xtrain)
end

# ╔═╡ bb6ef4d7-41e8-44b7-9035-8925f7b9dc6d
cm"- We obtain the posterior, conditioned on the (finite) observations, by calling posterior:"

# ╔═╡ 3f11045f-6aef-4b9a-bb42-ea27812f4861
function build_posterior_gp(θ)
    fx = build_finite_gp(θ)
    return posterior(fx, ytrain)
end

# ╔═╡ 855e09f4-2faf-42b9-a009-94a8324f33a8
cm"""
...
# ╔═╡ 1e951604-b4f4-4f1e-a028-a73696a4b23d
fpost_init = build_posterior_gp(ParameterHandling.value(θ_init))

# ╔═╡ 1d4a9af4-ec80-430a-b5f0-7587171fecf3
cm"- Let's visualize what the GP fitted to the data looks like, for the initial choice of kernel hyperparameters."
...
# ╔═╡ 1dd49150-46a0-4e52-b37a-f8db74088b8d
cm"""
...
# ╔═╡ 0fe4cadb-84dd-472e-b294-47e4f9a28290
function loss(θ)
    fx = build_finite_gp(θ)
...
begin
	...
	function optimize_loss(loss, θ_init; optimizer=default_optimizer, maxiter=1_000)
	    ...
	    result = optimize(NLSolversBase.only_fg!(fg!), θ_flat_init, optimizer, options; inplace=false)
	
	    return unflatten(result.minimizer), result
	end
end

# ╔═╡ 38c96e8f-9bc5-4161-a3b5-fa0d8840f198
cm"- We now run the optimization:"

# ╔═╡ 007792c2-ea15-4ad0-8ce5-fe51e07c6881
θ_opt, opt_result = optimize_loss(loss, θ_init)

# ╔═╡ 4553bca7-a86d-4a2f-b8ff-0b92126c1beb
cm"- Let's construct the posterior GP with the optimized hyperparameters:"
...
This notebook contains material obtained from [https://juliagaussianprocesses.github.io/AbstractGPs.jl/dev/examples/1-mauna-loa/](https://juliagaussianprocesses.github.io/AbstractGPs.jl/dev/examples/1-mauna-loa/).
"""
...
<table>
  ...
  <tr>
    ...
    <td><a href="./open?path=Lectures/Lecture-MachineLearning/Lecture-ML.jl">Lecture about ML tools</a></td>
  ...
  <tr>
    ...
    <td><a href="../../Lectures/Lecture-MachineLearning/Lecture-ML.html">Lecture about ML tools</a></td>
...
md"Notebook v1.1.1 - 21 September 2026"
...
manifest_format = "2.1"
project_hash = "9c7bbc16de973c25247ec1f524b4a378b42572d0"
