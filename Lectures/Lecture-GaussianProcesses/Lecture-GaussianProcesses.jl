...
# ╔═╡ 19c8d107-6899-4b7c-8d9e-495bae849dd3
WidthOverDocs()
...
- The formal definition of a GP is that the joint probability distribution over any finite sample ``y = \{y_i\}_{i=1,...,N}`` from the GP is a multi-variate Gaussian: 

```math
p(y) = \mathcal{N} (m, K)
``` 

- where ``m`` is the mean vector and ``K`` the covariance matrix.
...
```math
m_i = m(x_i, θ), \quad K_{i,j} = k(x_i, x_j , φ)
```

- where ``x_i`` is the set of inputs (independent variables) corresponding to the ith sample.
...
$(LocalResource("Pics/gp.png"))

- In the sketch above we show a probabilistic graphical model for GP regression given observation ``y`` at inputs ``t``, and a prediction at test input ``t_*``. Dots represent fixed variables (inputs), grey circles represent observed variables, white circls represent unknown (latent) variables. The thick horizontal bar indicates a set of fully connected nodes (meaning that every ``f`` depends on every ``t``). On the other hand, each observed output ``y_i`` depends only on the corresponding ``f_i`` and ``\sigma_i``, and is conditionally independent of the other variables. In this example, the measurement uncertainties ``\sigma`` and the GP hyper-parameters (not shown) are treated as known (fixed).
...
# ╔═╡ 32563555-4cac-47c7-b9a3-1eab1521c4aa
cm"""
...
- Let's consider ``N`` observations of a variable ``y = \{ y_i\}_{i=1, \ldots, N}``, taken at times ``t = \{ t_i\}``, with associated measurement uncertainties ``\sigma=\{\sigma_i\}``. 

-  We wish to compare these to a model function ``m(t,\theta)`` controlled by parameters ``\theta = \{\theta_j\}_{j=1,\ldots,M}``. 
...
- where ``m_i \equiv m(t_i,\theta)``, with respect to ``\theta``.
...
- Let us assume that the observations are given by `` y_i=m(t_i,\theta)+\epsilon_i``, where ``\epsilon_i`` is the measurement error, or noise, on the ``i^{\rm th}`` observation.
...
- Then the likelihood for the ``i^{\rm th}`` observation is simply:
...
- We also assume that the noise is uncorrelated, or white, meaning that the ``\epsilon_i``'s are drawn independently of each other from their respective distributions. 

- Then, the likelihood for the whole dataset ``y`` is merely the product of the likelihoods for the individual observations:

```math
\mathcal{L} (\theta) \equiv p(\mathbf{y}|\theta) = \prod_{i=1}^N \mathcal{L}_i = \prod_{i=1}^N \left\{ \frac{1}{\sqrt{2\pi} \sigma_i}
  \exp\left[-\frac{(y_i-m_i)^2}{2\sigma_i^2} \right] \right\} 
```
  
- One can readily see that ``\ln \mathcal{L} = \mathrm{constant} - 0.5 \chi^2``, where the constant depends only on the ``\sigma``'s.

- Thus, if the ``\sigma``'s are known, maximizing ``\mathcal{L}`` is equivalent to minimizing ``\chi^2``. 
...
# ╔═╡ 977d6057-59a6-4e4b-825c-4f707041cb95
cm"""
...
- where, the mean vector ``\mathbf{m}`` has elements ``m_i=m(t_i,\theta)`` and ``\mathbf{K}`` is a purely diagonal  (``N``, ``N``) *covariance matrix* of the model with elements ``K_{ij} = \delta_{ij} \sigma_i^2`` (``\delta_{ij}`` being the discrete Kronecker delta function). 
...
- where ``k`` is a *covariance function*, or *kernel function*, controlled by parameters ``\phi``. 
...
# ╔═╡ fcfb53d1-a51b-4016-97ba-9137bb6be7ab
cm"""
...
$(LocalResource("Pics/kernels.png"))
...
# ╔═╡ 90929813-a0b4-4c09-b940-d6522d792084
cm"""
### Making predictions by GPs
***

- Given some existing observations ``\mathbf{y}``, taken at times ``\mathbf{t}``, we can make predictions at some new set of times ``\mathbf{t}_\star``, i.e. computing ``p(\mathbf{y}_\star|\mathbf{y})``, the *conditional* probability distribution for ``\mathbf{y}_\star`` given ``\mathbf{y}``. 
...
```math
\mathbf{f}_\star = \mathbf{m}_\star + \mathbf{K}_\star^{\mathrm{T}} \, \mathbf{K}^{-1} \, (\mathbf{y} - \mathbf{m}) \qquad   \mathbf{C}_\star =   \mathbf{K}_{\star\star} - \mathbf{K}_\star^{\mathrm{T}} \, \mathbf{K}^{-1} \, \mathbf{K}_\star 
```

- where ``\mathbf{m}_\star \equiv m(\mathbf{t}_\star;\theta)``, ``[\mathbf{K}_\star]_{ij} = k(t_i,\mathbf{t}_{\star,j}; \phi)`` and ``[\mathbf{K}_{\star\star}]_{ij} = k(\mathbf{t}_{\star,i},\mathbf{t}_{\star,j};\phi)``.
"""

# ╔═╡ 858de066-5833-416a-8cb2-86d8ea91f0ca
cm"""
...
- Gaussians distributons own a set of very useful mathematical properties, e.g., the marginal distribution of any subset of elements from a multivariate normal distribution is also normal:
...
# ╔═╡ 6e6aa434-5ca7-45aa-a920-bbd7fde8c62a
cm"""
...
# ╔═╡ 2c042bee-e1e4-4a7b-9e57-99fecae3a743
function exponential_cov(x, y, prms)
    return prms[1] .* exp.( -0.5 .* prms[2] .* (x .- y').^2)
end

# ╔═╡ a679811a-158e-48c7-af62-c4521981d35c
cm"- This is instead the function implementing the conditional distribution:"

# ╔═╡ 55774ee2-9512-40d6-a3a6-7ecefddc90dc
function conditional(x_new, x, y, prms)    
    B = exponential_cov(x_new, x, prms)
    C = exponential_cov(x, x, prms)
    A = exponential_cov(x_new, x_new, prms)
    #mu = transpose(B) * inv(C) * y
    #sigma = A - transpose(B) * inv(C) * B
    mu = transpose(inv(C) * transpose(B)) * y
    sigma = A - B * inv(C) * transpose(B)
    return(mu,sigma)
end

# ╔═╡ 5d57cea2-244f-4666-b425-3a6978ff5138
cm"- Now, let's start with a Gaussian process prior with hyperparameters ``σ_0 = 1, σ_1 = 10``. We will also assume a zero function as the mean, so we can plot a band that represents one standard deviation from the mean."

# ╔═╡ 8f57dae1-94c4-48eb-ab2f-3dd13014a80a
begin
	θ = [1, 10]
	σ0 = exponential_cov(0, 0, θ)
	xpts = range(start=-3, stop=3, step=0.01)	
end;

# ╔═╡ c4c104fd-e752-4ab5-bcec-3605a8190445
begin
	fg1 = Figure()
	...
	scatter!(xpts,zeros(length(xpts)))
	errorbars!(xpts,zeros(length(xpts)),ones(length(xpts)) .* 60)
	
	fg1
end

# ╔═╡ d6867657-d3cc-49f3-b7b5-512ddc83550e
cm"""
...
# ╔═╡ 0ea9a0e1-638a-4ffd-9bd7-68af7c8f271e
begin
	d0 = Normal(0, σ0)
	x = [1.,]
	y = rand(d0,1)
...
# ╔═╡ a80f6c27-9db7-43f8-bfe1-fc4126474f1f
cm"- We can now update our confidence band, given the point that we just sampled, using the covariance function to generate new point-wise intervals, conditional on the value ``[x_0, y_0]``:"

# ╔═╡ bed77c41-c738-4872-8490-a95a4e74f524
σ1 = exponential_cov(x, x, θ);

# ╔═╡ d3b74cf2-f3b3-498c-8034-8deff9de9f17
cm"- And now let's write a function to compute the predictions of our GP:"

# ╔═╡ 2baa9e37-1055-4137-92ec-222534a9fedc
function predict(x, data, kernel, prms, sigma, t)
    k = [kernel(x, y, prms) for y in data]
    Sinv = inv(sigma)
    y_pred = transpose(k) * Sinv * t
    sigma_new = kernel(x, x, prms) - transpose(k) * Sinv * k
    return y_pred, sigma_new
end

# ╔═╡ ee589087-d370-4edd-9611-4bcd813325d4
begin
	x_pred = range(start=-3., stop=3., length=1000)
	predictions = [predict(i, x, exponential_cov, θ, σ1, y) for i in x_pred]
end;

# ╔═╡ fa2c7d48-d992-4358-a550-7f444a56a183
begin
	y_pred = [l[1][1] for l in predictions]
	sigmas = [l[2][1] for l in predictions]
	...
	errorbars!(x_pred,y_pred,sigmas)
	scatter!(x,y,color=:red)
	
	fg2
end

# ╔═╡ 7f3b5dcf-f396-4d0c-9d90-b6afae2c1553
cm"- So conditional on this point, and the covariance structure we have specified, we have essentially constrained the probable location of additional points. Let's now sample another:"

# ╔═╡ 5dba0071-e062-448e-91f4-6f987510fc25
begin
...
# ╔═╡ ff220252-4781-4fa3-a376-2afb3df2fdc2
begin
	push!(x,-0.7)
	push!(y,yn[1])
	σ2 = exponential_cov(x, x, θ)
	predictions2 = [predict(i, x, exponential_cov, θ, σ2, y) for i in x_pred]
end;

# ╔═╡ 15b5ffd2-fd73-452e-b6c0-d1869f360d60
begin
	y_pred3 = [l[1][1] for l in predictions2]
	sigmas3 = [l[2][1] for l in predictions2]
	...
	errorbars!(x_pred,y_pred3,sigmas3)
	scatter!(x,y,color=:red)
	
	fg3
end

# ╔═╡ c2625882-cdab-437d-b99e-a3b3be35ed9e
cm"- Of course, sampling sequentially is just a heuristic to demonstrate how the covariance structure works. We can just as easily sample several points at once:"

# ╔═╡ e6a27cc5-0a3d-4b55-a455-4bfe083a405e
begin
	x_more = [-2.1, -1.5, 1.2, 1.8, 2.5]
	m_more, s_more = conditional(x_more, x, y, θ)
	dm = Normal.(m_more,s_more)
	y_more = rand(d0, length(m_more))
	println(y_more)
end;

# ╔═╡ 0a3493f1-6338-48ae-aa79-6af7102bbcde
begin
	xfin = vcat(x,x_more)
	yfin = vcat(y,y_more)
	σ_new = exponential_cov(xfin, xfin, θ)
	predictions_more = [predict(i, xfin, exponential_cov, θ, σ_new, yfin) for i in x_pred]
end;

# ╔═╡ 4d7629db-fbcf-4551-85bb-5e5826f96c5d
begin
	y_pred4 = [l[1][1] for l in predictions_more]
	sigmas4 = [l[2][1] for l in predictions_more]
	...
	errorbars!(x_pred,y_pred4,sigmas4)
	scatter!(xfin,yfin,color=:red)
	
	fg4
end

# ╔═╡ 55e7a148-7929-4668-869c-3c1835099d08
cm"- Essentially, as the density of points becomes high, it results in a realization (sample function) from the prior GP."

# ╔═╡ 40cd9630-821f-4c8c-bf6a-119b59560c9c
cm"""
### GPR Inference workflow
***

$(LocalResource("Pics/gpinference.png"))
"""

# ╔═╡ a5c28c41-fee7-4602-ba0a-2519b97ee4e5
cm"""
### Exercize: model fitting with correlated noise
***
...
- The model that we analyse in this exercize is a single Gaussian feature with three parameters: amplitude α, location ℓ,  and width σ``^2``. This is qualitatively similar to a few problems in astronomy and physical sciences in general, e.g., fitting spectral features, measuring transit times, etc.


- Let’s start by simulating a dataset of 50 points with known correlated noise using parameters: α=−1,ℓ=0.1, and σ``^2``=0.4.
"""

# ╔═╡ b6b4b1e9-a5cc-4fa4-a2c6-7e389d82f6d2
# The "feature" in our data set
#
function fundmod(x, α, l, σ2)
    return α * exp.(-(x .- l).^2 ./ (2*σ2))
end;

# ╔═╡ 9507a2d0-cd5d-4020-8f76-a5f428c6e5d7
begin
	# A GP with "squared exponential" kernel and "fundmod" as mean function.
	# We use the Stheno package
	#
	Amp = 0.1
	Lung = 3.3
	k = Amp * with_lengthscale(SqExponentialKernel(), Lung)
	f = atomic(GP(x -> fundmod(x,-1.,0.1,0.4),k), GPC())
end;

# ╔═╡ d4d9e526-4b3a-4abf-a458-400a8f128297
cm"- Let's also define a seed for the random number generator for reproducibility"

# ╔═╡ 9e6f7029-906c-428a-aa21-1cc458d45b1b
Random.seed!(6);

# ╔═╡ 326663c2-5eb2-47d8-8fb1-7574aa1cf870
# The function to generate data
#
function generate_data(params, N, rng=[-5, 5])
    t = rng[1] .+ diff(rng) .* sort(rand(N))
    y = rand(f(t,1e-6))
    yerr = 0.05 .+ 0.05 .* rand(N)
    y += yerr .* randn(N)
    return t, y, yerr
end;

# ╔═╡ de2db225-fe88-4686-a632-5d01da63a407
begin
	truth = [-1.,0.1,0.4]
	tdata, ydata, ydataerr = generate_data(truth, 50)
end;

# ╔═╡ 39a8a98c-4207-438a-89df-aa603aa0f482
cm"- And let's see what we got:"

# ╔═╡ cc5f67f9-aaeb-4571-8918-60a71833283b
begin
	fggp1 = Figure()
	
	ax1gp1 = Axis(fggp1[1, 1],
	    xlabel="t",
	    ylabel="y",
	...
	scatter!(tdata,ydata)
	errorbars!(tdata,ydata,ydataerr)
	
	fggp1
end

# ╔═╡ 6de0a354-dc3b-43f2-b58f-cd4119390c1c
cm"""
...
# ╔═╡ bb6770b0-f615-475a-b143-3fd2470305ea
function fitmodel(x,θ)
    m,b,α,l,σ2 = θ
    return b .+ m .* x .+ fundmod(x, α, l, σ2)
end;

# ╔═╡ 6c2f7d04-a296-458a-aa79-09e0dca316e2
cm"- And, as usual, let's define a probabilistic model for our problem. We adopt uniform priors for simplicity. In a real problem choosing adequate priors would be a fundamental step for a properly carried out analysis."

# ╔═╡ 5d504def-6098-49a1-9be3-09a4ec1047da
begin
	@model function NoKernelModel(t,y,ey)
	    m ~ Uniform(-1,1)
	    b ~ Uniform(-1,1)
	    α ~ Uniform(-5,0)
	    l ~ Uniform(-1,1)
	    lσ2 ~ Uniform(-10,0)
	    #
	    fmod = fitmodel(t,[m,b,α,l,exp.(lσ2)])
	    #
	    return y ~ MvNormal(fmod,ey)
	end
	
	tmod = NoKernelModel(tdata,ydata,ydataerr);
end;

# ╔═╡ a3a6d288-f129-48bb-be4c-8b4e4142b272
cm"- Now, we run a MCMC chain:"

# ╔═╡ 9a459b07-0ab6-4e7a-865b-1d6627a137dc
begin
	iterations = 2000
	burnins = 1000
	chains = 4
	
	chain = mapreduce(c -> sample(tmod, NUTS(burnins,0.6), iterations), chainscat, 1:chains)
end

# ╔═╡ 2ebada5c-5a16-483a-a6de-0e4c8965b70b
begin
	# Let's create a dataframe with the chain results
	#
	dfc = DataFrame(chain)
	dfc[!,:σ2] = exp.(dfc[!,:lσ2])
end;

# ╔═╡ 474f8909-d7af-4da1-8586-750b9683ed79
cm"- After running the chain, we can plot the predicted results extracting a random sample on top of the data:"

# ╔═╡ cfae66b5-ce01-4da2-b037-b7758e1c0e17
begin
	fggp2 = Figure()
	
	ax1gp2 = Axis(fggp2[1, 1],
	    xlabel="t",
	    ylabel="y",
	    title="Simulated data"
	    )
	...
	tpl = range(start=minimum(tdata),stop=maximum(tdata),length=100)
	
	for i in rand(1:nrow(dfc),100)
	    yc = fitmodel(tpl,[dfc[i,:m],dfc[i,:b],dfc[i,:α],dfc[i,:l],dfc[i,:σ2]])
	    lines!(tpl,yc,color=:orange,alpha=0.1)
	end
...
# ╔═╡ 317be9ad-1672-485f-a095-e84c5e2b822c
cm"""
- At least visually, the fit seems to be satisfactory.

- Nevertheles, we can check the posterior PDF of the parameters with a regular "corner plot":
"""

# ╔═╡ d9d7ff99-7cc8-4b16-bf6d-b9e3ad3869a7
begin
	dfcs = DataFrames.select(dfc, [:α,:l,:σ2])
	pairplot(dfcs,PairPlots.Truth((α = truth[1],l = truth[2],σ2 = truth[3])),labels = Dict(:α => "α",:l => "l",:σ2 => L"σ$^2$"))
end

# ╔═╡ 15bd2d40-c0ed-43aa-9cf9-88ce7a8385de
cm"- Not fully surprinsingly, the most probable values are not fully consistent with the *true* values we used in generating our data."

# ╔═╡ d7da0dbd-77dd-4df0-b0b6-1b84826b0d39
cm"""
#### Modeling the Noise
***
...
- where ``δ_{ij}`` is the Kronecker_delta and ``k(⋅,⋅)`` is a covariance function that we get to choose. For this exercize, we’ll just use the Matérn-3/2 function:
...
# ╔═╡ b248f3c0-a419-47c1-b47e-d58a93c987fd
begin
	# Now we include a GP in the model
	#
	@model function KernelModel(t,y,ey)
	    α ~ Uniform(-5,0)
	    l ~ Uniform(-1,1)
	    lσ2 ~ Uniform(-10,0)
	    lamp ~ Uniform(-10,10)
	    llung ~ Uniform(0,10)
	    #
	    k = exp(lamp) * with_lengthscale(Matern32Kernel(), exp(llung))
	    fmod = atomic(GP(t -> fundmod(t,α,l,exp.(lσ2)),k), GPC())
	    fx = fmod(t,1e-6 .+ diagm(ey.^2))
	    #
	    return y ~ fx
	end
	
	tkmodgp = KernelModel(tdata,ydata,ydataerr);
end;

# ╔═╡ 14488f5a-60ef-48ae-8183-28c72b485d10
begin
	iterationsgp = 2000
	burninsgp = 1000
	chainsgp = 4
	
	kchaingp = mapreduce(c -> sample(tkmodgp, NUTS(burninsgp,0.6), iterationsgp), chainscat, 1:chainsgp)
end

# ╔═╡ a95430eb-d4c8-4cbf-86b0-22467d658ee4
begin
	kdfcgp = DataFrame(kchaingp)
	kdfcgp[!,:σ2] = exp.(kdfcgp[!,:lσ2])
end;

# ╔═╡ defade6c-0719-4afd-871d-83e223aec195
begin
	fggp3 = Figure()
	
	ax1gp3 = Axis(fggp3[1, 1],
	    xlabel="t",
	    ylabel="y",
	    title="Simulated data"
	    )
	...
	tplgp = range(start=minimum(tdata),stop=maximum(tdata),length=100)
	
	for i in rand(1:nrow(kdfcgp),100)
	    kgp = exp(kdfcgp[i,:lamp]) * with_lengthscale(Matern32Kernel(), exp(kdfcgp[i,:llung]))
	    fmodgp = atomic(GP(tdata -> fundmod(tdata,kdfcgp[i,:α],kdfcgp[i,:l],kdfcgp[i,:σ2]),kgp), GPC())
	    f_postgp = posterior(fmodgp(tdata, 1e-6 .+ diagm(ydataerr.^2)), ydata)
	    ms_optgp = marginals(f_postgp(tplgp))
	    ycgp = mean.(ms_optgp)
	    lines!(tplgp,ycgp,color=:orange,alpha=0.1)
	end
...
# ╔═╡ bbc64f4b-5731-41e2-a5ef-0ece9f59b8c3
cm"- Again the fit is visually appealing, please note we did not use the same kernel used to generate the data to make the example a bit less contrived."

# ╔═╡ e43a5b97-1a14-45b3-acbe-911bc7d36d6c
begin
	kdfcsgp = DataFrames.select(kdfcgp, [:α,:l,:σ2])
	pairplot(kdfcsgp,PairPlots.Truth((α = truth[1],l = truth[2],σ2 = truth[3])),labels = Dict(:α => "α",:l => "l",:σ2 => L"σ$^2$"))
end

# ╔═╡ 81eb3935-42db-4af0-b369-1bad77e6a39c
cm"- And now, finally, the *true* parameters are very close to those obtained by the analysis."

# ╔═╡ 96303d0f-9f57-4ab0-9fb1-c16ecee3ea73
cm"""
## Multivariate GPs
***
...
- As a matter of fact, most of our discussion holds even for multivariate datasets.
    ...
    - However, when working with univariate GPs, it is relatively straightforward and unambiguous to compute the “distance” between two points ``t_i``, and ``t_j``: ``ρ(t_i,t_j)=τ=|t_i−t_j|``.

    - For multivariate inputs ``x``, more care must be taken to define a sensible distance metric ``ρ(x_i,x_j)``.
...
- One useful way to specify these datasets are to define the inputs as ``x_i ≡ (t_i,l_i)``, where ``t_i`` is the time of the i-th observation, and ``l_i`` is the “label” for which time series the i-th observation is drawn from.

- In this case, one common choice of kernel function is:

```math
k(x_i, x_j; \phi) = a_{l_i}^T a_{l_j} k_0(t_i, t_j; \phi) 
```

- where ``k_0(t_i,t_j; φ)`` is a standard one-dimensional kernel, and the set of ``\{a_d\}^D_{d=1}``, with ``D`` the total number if input time-series, are also hyper-parameters of the model.

- A simple technique implementing this idea is known as *intrinsic model of coregionalization* ([Covino et al. 2022](https://ui.adsabs.harvard.edu/abs/2022MNRAS.513.2841C/abstract)), although many other, often more sophisticated, approaches are possible.

- Assuming we have ``D`` different input data sets (or bands, as it is often the case in astronomy) the kernel can be defined as: `` K_{\rm coreg} = k(x,x') \cdot B[i,j]``, where ``x`` and ``x'`` are observations from any input data sets, ``k(x,x')`` is a valid covariance function, and ``B`` is a ``P \times P`` positive-definite matrix:

```math
K_{\rm coreg} =
\begin{bmatrix}
    B_{11} k(X_1,X_1) & \dots & B_{1D} k({X_1,X_D}) \\
    \vdots & \ddots & \vdots \\
    B_{P1} k(X_D,X_1) & \dots  & B_{DD} k(X_D,X_D) 
\end{bmatrix}.
```

- Each output can be seen as linear combination of functions defined by the same covariance ``k(x,x')``. The rank of the ``B`` matrix defines the number of component in the linear combination. Therefore, an ICM is generated by the products of two covariance functions: *one that models the dependence between the outputs and one that models the input dependence*.

    - If ``B_{ij} = 0`` for ``i \ne j`` describes the case with independent data for bands ``i`` and ``j``. 


$(LocalResource("Pics/mvgp.png"))
"""
...
- [Aigrain & Foreman-Mackey (2023) - "Gaussian Process Regression for Astronomical Time Series”](https://ui.adsabs.harvard.edu/abs/2023ARA%26A..61..329A/abstract)
- [Covino et al. (2022) - "Detecting the periodicity of highly irregularly sampled light curves with Gaussian processes: the case of SDSS J025214.67-002813.7"](https://ui.adsabs.harvard.edu/abs/2022MNRAS.513.2841C/abstract)
"""

# ╔═╡ 7e5ec401-6cea-4a43-974b-0a3069765017
cm"""
## Further Material

Papers for examining more closely some of the discussed topics.

- [Covino et al. (2020) - "Looking at Blazar Light-curve Periodicities with Gaussian Processes"](https://ui.adsabs.harvard.edu/abs/2020ApJ...895..122C/abstract) 
- [Deisenroth et al. (2020)- "A Practical Guide to Gaussian Processes"](https://infallible-thompson-49de36.netlify.app/)
- [O'Sullivan & Aigrain (2024) - "Modelling stochastic and quasi-periodic behaviour in stellar time-series: Gaussian process regression versus power-spectrum fitting"](https://ui.adsabs.harvard.edu/abs/2024MNRAS.531.4181O/abstract)
- [Williams & Rasmussen (2005) - "Gaussian Processes for Machine Learning"](http://gaussianprocess.org/gpml/chapters/RW.pdf)

"""

# ╔═╡ 8b57c8c3-fdc2-48be-93fb-1c2a5ca0eeb3
cm"""
### Credits
***

This notebook contains material obtained from [https://www.dominodatalab.com/blog/fitting-gaussian-process-models-python](https://www.dominodatalab.com/blog/fitting-gaussian-process-models-python) and [https://george.readthedocs.io/en/latest/tutorials/model/](https://george.readthedocs.io/en/latest/tutorials/model/).
"""
...
# ╔═╡ e9b51adb-8861-474c-bae5-b40a565c852c
md"Notebook v1.1.0 - 18 September 2026"
...
manifest_format = "2.1"
project_hash = "e557c2d2ddde48a4651ea6fbff77556243302d46"
...
[[deps.AbstractGPsMakie]]
deps = ["AbstractGPs", "LinearAlgebra", "Makie"]
git-tree-sha1 = "4ed586f3c57c5644680d5ae34c2ef7e480cbb1bf"
registries = "General"
...
git-tree-sha1 = "14ded43f8a62568b17efc62d9f04491a2d4d8c9b"
registries = "General"
...
git-tree-sha1 = "8a6ffa41b9c56aeb88c2962b12f35dd18ea2623f"
registries = "General"
...
git-tree-sha1 = "aae458967ad592c394975c2751f5a441ab72891e"
registries = "General"
...
git-tree-sha1 = "d9f7c3abfe41bf119c4d8848016f0f113c89ef11"
registries = "General"
...
git-tree-sha1 = "05e4dd6cfcaaa38e423c054a82aa3a43abd9a5b3"
registries = "General"
