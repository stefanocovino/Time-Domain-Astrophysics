...
# ╔═╡ a2e7d556-5132-11f1-9ee4-dd132c0065f0
using CSV, DataFrames, CairoMakie, LombScargle, Statistics


# ╔═╡ a46b7234-f48a-4a4b-b876-e70de57e57ee
# ╠═╡ disabled = true
#=╠═╡
# Read the CSV
df = CSV.read("Algol.csv", DataFrame)	
  ╠═╡ =#

# ╔═╡ 5e3e1770-d492-43f3-9b1d-ced7affe2c8a
# Ensure numeric types at read time
df = CSV.read("Algol.csv", DataFrame,
    types = Dict(:Magnitude => Float64, :Uncertainty => Float64, :JD => Float64),
    missingstring = "")

# ╔═╡ 44f9d7d7-f73a-41de-86da-261b6a398363
begin
	# --- Filter by Band ---
	selected_band = "V"   # change this to any band you want
	df_filtered = filter(row -> row.Band == selected_band, df)
	df_filtered = dropmissing(df_filtered, [:JD, :Magnitude, :Uncertainty, :Band])
end

# ╔═╡ a1d5be08-752c-42a7-bba4-e87fac08fa9f
begin
	...
	ax = Axis(fig[1, 1],
	    ...
	    ylabel = "Magnitude",
	    title = "Algol - V band",
	    yreversed = true)   # magnitudes increase downward
	
	# Error bars
	errorbars!(ax, df_filtered.JD, df_filtered.Magnitude, df_filtered.Uncertainty,
	    color = (:steelblue, 0.6),
	    whiskerwidth = 4)
	
	# Data points
	scatter!(ax, df_filtered.JD, df_filtered.Magnitude,
	    color = :steelblue,
	    markersize = 6)
...
# ╔═╡ aef9138f-be0e-4744-95b3-9f58253bf16f
begin
	sort!(df_filtered, :JD)
	
	t   = df_filtered.JD
	mag = df_filtered.Magnitude
	err = df_filtered.Uncertainty
end

# ╔═╡ e7aa825d-64ad-472a-b828-c3fbdfc8594b
begin
	plan = LombScargle.plan(t, mag, err;
	    oversampling  = 5,        # finer frequency grid
	    minimum_frequency = 0.3,   # one cycle over full baseline
	    maximum_frequency = 0.5)  # Nyquist-like upper limit; adjust to your data
	
	pgram = lombscargle(plan)
	
	# --- Best period ---
	best_freq, best_power = findmaxfreq(pgram), findmaxpower(pgram)
	best_period = 1.0 / best_freq[1]
	#best_period = 2.8673
	println("Best period: $(round(best_period, digits=4)) days  |  Power: $(round(best_power, digits=4))")
	
	# False alarm probability at the peak
	fap = LombScargle.fap(pgram, best_power)
	println("FAP at peak: $fap")
	
end

# ╔═╡ 98e036a1-0f90-44bb-a2e5-b45a617ba1cd
begin
	figls = Figure(size = (900, 700))
	
	# Periodogram
	ax1ls = Axis(figls[1, 1],
	    xlabel = "Period (days)",
	    ylabel = "LS Power",
	    title  = "Lomb-Scargle Periodogram — $(selected_band)")
	
	freqs   = freqpower(pgram)[1]
	powers  = freqpower(pgram)[2]
	periods = 1.0 ./ freqs
	
	lines!(ax1ls, periods, powers, color = :steelblue, linewidth = 1.0)
	
	# Mark best period
	vlines!(ax1ls, best_period, color = :crimson, linestyle = :dash, linewidth = 1.5)
	text!(ax1ls, best_period, maximum(powers) * 0.95;
	    text  = "P = $(round(best_period, digits=3)) d",
	    align = (:left, :top),
	    color = :crimson,
	    fontsize = 13)
	...
	ax2ls = Axis(figls[2, 1],
	    xlabel = "Phase",
	    ylabel = "Magnitude",
	    title  = "Phase-folded at P = $(round(best_period, digits=3)) days",
	    yreversed = true)
	
	phase = mod.((t .- t[1]) ./ best_period, 1.0)
	
	errorbars!(ax2ls, phase, mag, err,
	    color = (:steelblue, 0.5), whiskerwidth = 4)
	scatter!(ax2ls, phase, mag,
...
manifest_format = "2.0"
project_hash = "7daaf5244d940ecbb6b095661df40f129012606c"
