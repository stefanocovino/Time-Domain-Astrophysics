...
# ╔═╡ 0e394e9e-9d0a-4a77-9418-b1704413e66a
WidthOverDocs()  

# ╔═╡ 73c52bd4-35d3-4176-b94a-46b4cfb9ba61
TableOfContents()
...
# ╔═╡ 9ea02ca1-5b69-4fdf-9ecf-531ecd91693f
Markdown.parse("""

##### Sampling, mean: $(latexify(dt,fmt="%.5f"))s, variance: $(latexify(dtvar,fmt="%.5f"))s
...
md"""
- A powerful peak is clearly visible. Again let's zoom in.
"""
...
# ╔═╡ e4787377-dcd8-4394-ad7e-93fb3a4d9b75
Markdown.parse("""

##### Maximum power: $(latexify(pmax,fmt="%.2f")) at frequency $(latexify(fmax,fmt="%.2f")) in a $(latexify(length(rxtefr),fmt="%d")) frequency bin long periodogram.

""")

# ╔═╡ 446c8dfe-c97c-4e64-a6c4-46ccf91880f9
...
- And let's also assume our input time-serien consists of 200 points, generating therefore a 100 frequency bin periodogram.
- Ledt's compute the final significance:- Ledt's compute the final significance:
"""
...
# ╔═╡ f2ed5b72-a318-4731-a65a-43bc260c2859
Markdown.parse("""

##### FAP: $(latexify(eps*100,fmt="%.2f"))%

##### Single-trial probability: $(latexify(Ppre*100,fmt="%.2f"))%

##### Post-trial probability: $(latexify(Ppost*100,fmt="%.2f"))%

""")

# ╔═╡ f4106f3c-9d48-4016-8249-c5bef66f198d
md"""
- Quite interestingly, the final probability can justify a claim of a hint of periodicity, but nothing more: high significance periodicities need to be of very high single-trial significance.
...
# ╔═╡ cab56474-22c7-449d-ae22-2e20bd95b5a1
Markdown.parse("""

##### Single-trial probability: $(latexify(100*(1-probmax),fmt="%.2f"))%

##### Post-trial probability: $(latexify(100*postprobmax,fmt="%.2f"))%
...
md"""
- We see that computing the periodogram in 70 10s bins the drifting of the QPO frequency is clearly visible.
"""
...
# ╔═╡ 05a76efe-66af-4a23-b09e-741d10927d15
md"Notebook v1.1.0 - 7 September 2026"
...
manifest_format = "2.0"
project_hash = "5ab4073f7ccbfb160fbac04f1acc8b8595057488"
