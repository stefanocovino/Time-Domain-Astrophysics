...
# ╔═╡ 7cc828aa-f4fd-4ba6-9bdf-3606a60cfaf0
WidthOverDocs()
...
- The $0.001 \times (t - 100)^2$ term defines the (parabolic) trend of the series,  $2\sin(\frac{2\pi t}{p_1})$ and $0.75\sin(\frac{2\pi t}{p_2})$ are two periodic components with differing periodicities and amplitudes.
...
- The first step of SSA is to map the time series ``F`` to a sequence of multi-dimensional lagged vectors. 

- Let an integer ``L`` be the *window length*, ``2 \le L \le N/2``. We form a 'window', given by the subseries ``\{f_i, \ f_{i+1}, \ldots , \ f_{i+L-1}\}``, for  ``i=0,\ldots,N-L``. 

- We slide this window along the time series, forming a column vector, ``X_i``, for each subseries. That is, we have:
...
- From writing out the matrix above, it is clear that the elements of the *anti-diagonals* (that is, the diagonals running from bottom-left to top-right) are equal. This type of matrix is known as a [Hankel](https://en.wikipedia.org/wiki/Hankel_matrix) matrix.

- For our toy time series, we'll set the window length to 70, and defer discussion on how to select an appropriate window length later. 

- Let ``K = N - L + 1`` represent the number of columns in the trajectory matrix. We'll refer to the columns of ``\mathbf{X}`` as the ``L``-lagged vectors, and the rows as ``K``-lagged vectors.
"""
...
- The [Frobenius norm](https://en.wikipedia.org/wiki/Matrix_norm#Frobenius_norm) of ``\mathbf{X}``, ``\lvert\lvert \mathbf{X} \rvert\rvert_{\text{F}}``, is given by
```math
...
- Even without inspecting the ``U_i`` vector associated with each ``\mathbf{X}_i``, or reconstructing a time series of each component, the appearance of the ``\mathbf{X}_i`` hints at the nature of each component, be it trend, periodicity or noise.
    ...
    - For example, the ``L``- and ``K``-lagged vectors in ``\mathbf{X}_1`` and ``\mathbf{X}_2`` vary relatively slowly across the matrix, suggesting that ``\mathbf{X}_1`` and ``\mathbf{X}_2`` may be associated with the overall trend in the time series.
    - The matrices ``\mathbf{X}_3`` to ``\mathbf{X}_6`` show large checkerboard patterns, suggesting periodicity.
...
md"""
- The plots above depict the relative and cumulative contributions of the first 12 ``\mathbf{X}_i`` in the expansion ``\mathbf{X} = \sum_{i=1}^{d}\mathbf{X}_i``.

- The elementary matrices ``\mathbf{X}_1`` and ``\mathbf{X}_2`` contribute 52% and 22%, respectively, to the expansion of  ``\mathbf{X}``. Together, the first seven elementary matrices contribute 97%.

- Elementary matrices that make equal contributions to the expansion (that is, ``\sigma_i \approx \sigma_{i+1}``) are likely to be grouped together when reconstructing the time series, and appear as "breaks" in the plot of relative contributions. For example, the "breaks" in the plot above suggest that ``\mathbf{X}_3`` and ``\mathbf{X}_4``, and ``\mathbf{X}_5`` and ``\mathbf{X}_6``, should be grouped together. 

> It is important to note that the elementary matrices represent an optimal (although possibly non-unique) separation of components in trajectory space: by definition, the rows and columns of one elementary matrix are orthogonal to the rows and columns of the other elementary matrices. 

- However, this separation may not (necessarily) coincide with what we would consider a useful, interpretable 'component' of the time series. In fact, there are restrictions on the types of time series components that are exactly separable under this formalism.
"""
...
- Inspection of the Hankelised elementary matrices of the toy time series confirms our suspicians about the elementary matrices:
	...
	- ``\tilde{\mathbf{X}}_1`` and ``\tilde{\mathbf{X}}_2`` vary slowly over the whole time series, and can be grouped together as the trend component.
	- ``\tilde{\mathbf{X}}_3`` and ``\tilde{\mathbf{X}}_4`` are both periodic, with the same frequency, and can be grouped as the first periodic component.
	- ``\tilde{\mathbf{X}}_5`` and ``\tilde{\mathbf{X}}_6`` are also periodic, with a different frequency to ``\tilde{\mathbf{X}}_3`` and ``\tilde{\mathbf{X}}_4``, and will be grouped as the second periodic component.
	- ``\tilde{\mathbf{X}}_7``, lacking obvious periodicity, will be grouped with the trend components.

- We'll lump together all components from ``\tilde{\mathbf{X}}_8`` and beyond as noise.
    
- To summarise:
```math
\begin{align*}
\tilde{\mathbf{X}}^{\text{(trend)}} & = \tilde{\mathbf{X}}_1 + \tilde{\mathbf{X}}_2 + \tilde{\mathbf{X}}_7 
    & \implies &  \tilde{F}^{\text{(trend)}} = \tilde{F}_1 + \tilde{F}_2 + \tilde{F}_7 \\
\tilde{\mathbf{X}}^{\text{(periodic 1)}} & = \tilde{\mathbf{X}}_3 + \tilde{\mathbf{X}}_4 
    & \implies & \tilde{F}^{\text{(periodic 1)}} = \tilde{F}_3 + \tilde{F}_4  \\
\tilde{\mathbf{X}}^{\text{(periodic 2)}} & = \tilde{\mathbf{X}}_5 + \tilde{\mathbf{X}}_6 
    & \implies & \tilde{F}^{\text{(periodic 2)}} = \tilde{F}_5 + \tilde{F}_6\\
\tilde{\mathbf{X}}^{\text{(noise)}} & = \tilde{\mathbf{X}}_8 + \ldots + \tilde{\mathbf{X}}_{70}
    & \implies & \tilde{F}^{\text{(noise)}} = \tilde{F}_8 + \ldots + \tilde{F}_{70}
\end{align*}
```
...
- As mentioned earlier, the elementary components separated in the time series' trajectory space may not coincide with a single, interpretable component in the time series.
    - For example, ``\tilde{F}_1$ and $\tilde{F}_2`` both look vaguely like the trend— are they *really* separate components? Similarly, ``\tilde{F}_3`` and ``\tilde{F}_4`` are almost identical, except near the boundaries of the time series.
...
md"""
- The structure of $\mathbf{W}_{\text{corr}}$ shows a lot of correlation between the time series components, particularly in the range $8 \le i,j \le 70$. As these were the components we classified as belonging to the noise in the time series, it is no surprise that there are non-negligible correlations between all of them; this is a natural result of the noise having no underlying structural component that can be further separated.

- It is important to note that $\mathbf{W}_{\text{corr}}$ is roughly split into two 'blocks': $1 \le i,j \le 7$, and $8 \le i,j \le 70$. This corresponds to two main groupings: a smoothed time series (i.e. the trend plus the two periodic components), and the residual noise. Zooming into the first seven components in $\mathbf{W}_{\text{corr}}$:
"""
...
md"""
- Interestingly, the trend component ``\tilde{F}^{(1)}`` has started to deteriorate at ``L=40``, with notable 'kinks' in the time series.
