...
# ╔═╡ 2bacae2d-479e-4231-8c45-d0f126c1bba0
include("src/SSAJL.jl");
...
# ╔═╡ bd99514c-7a10-44dd-92ad-ed184e0a6651
begin
	walk_1 = DataFrame(CSV.File("A_DeviceMotion_data/wlk_15/sub_1.csv"))
	first(walk_1,5)
end
...
# ╔═╡ 7b5a0cf2-3a43-48c4-8e01-85f0b03c17f5
begin
	cols = ["userAcceleration.x", "userAcceleration.y", "userAcceleration.z"]
	accel_1 = sqrt.(sum(eachcol(walk_1[!,cols].^2)))
	index_1 = collect(1:nrow(walk_1))./50
end;

# ╔═╡ c39c8f7a-68a2-4cc4-a759-737f2272f076
cm"- Let's plot the time series for the magnitude of the acceleration:"

# ╔═╡ eaaaeb43-1562-44bc-a849-ca9aa3a34b5c
begin
	fg = Figure()
	...
	ax = Axis(fg[1, 1],
	    ...
	    title = "Magnitude of Acceleration Vector While Walking",
	)
	
	lines!(index_1,accel_1)
	
	fg
end

# ╔═╡ a61c9e0f-f0e0-4cc8-8ae3-879433b7f694
cm"""
...
begin
	...
	axfg2 = Axis(fg2[1, 1],
	    xlabel = "t (s)",
	    ylabel = "Acceleration (g)",
	    title = "Example of Periodicity in Walking Time Series",
	)
	
	trng = (index_1 .>= 4) .& (index_1 .<= 5)
	lines!(index_1[trng],accel_1[trng])
	
	saccel_1 = ShiftedArrays.lead(accel_1, 56)
	lines!(index_1[trng],saccel_1[trng])
	
	ssaccel_1 = ShiftedArrays.lead(accel_1, 112)
	lines!(index_1[trng],ssaccel_1[trng])
	
	sssaccel_1 = ShiftedArrays.lead(accel_1, 168)
	lines!(index_1[trng],sssaccel_1[trng])
...
begin
	...
	rng = (index_1 .>= tstart) .& (index_1 .<= tend)
...
accel_ssa = SSAJL.SSA(accel_1[rng], window);

# ╔═╡ 861ac23a-dbc5-4d87-95e7-4fe03a53daca
cm"- As always, plot the w-correlation matrix first to help orient ourselves with the separated components."

# ╔═╡ 0d958b0f-449a-4dd1-bbbe-3871117db734
SSAJL.plot_wcorr(accel_ssa,ptitle="W-Correlation for Walking Time Series")
...
begin
	...
	axfg4 = Axis(fg4[1, 1],
	    xlabel = "t",
	    ylabel = "Acceleration (g)",
	    title = "Walking Time Series: First Three Groups",
	)
	
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,1), label=CairoMakie.L"\tilde{F}^{(1)}")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,2:3), label=CairoMakie.L"\tilde{F}^{(2)}")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,4:5), label=CairoMakie.L"\tilde{F}^{(3)}")
	lines!(index_1[rng], accel_1[rng], label="Original time-series")
	
	axislegend(axfg4,framevisible = false,position = :ct)
...
begin
	...
	axfg5 = Axis(fg5[1, 1],
	    xlabel = "t",
	    ylabel = "Acceleration (g)",
	    title = "Walking Time Series: First Three Groups (Zoomed)",
	)
	...
	axislegend(axfg5,framevisible = false,position = :ct,orientation = :horizontal)
	
	fg5
end

# ╔═╡ 4cedbdb5-4162-4d2d-bb09-13054aeab4b5
cm"""
...
# ╔═╡ c1637480-03b6-479a-bce1-c725cf8d6644
begin
	fg6 = Figure()
	...
	axfg6 = Axis(fg6[1, 1],
	    xlabel = "t",
	    ylabel = "Acceleration (g)",
	    title = "Walking Time Series: Low-Frequency Periodicity",
	)
	
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,1:5), label=CairoMakie.L"\tilde{F}^{(1)}")
	lines!(index_1[rng], accel_1[rng], label="Original time-series")
	
	xlims!(16,20)
	ylims!(-0.5, 2.5)
	
	#CairoMakie.axislegend(ax,framevisible = false,position = :ct,orientation = :horizontal)
	
	fg6
end

# ╔═╡ b3f4e1c3-e5b1-4838-a432-8421413cbac4
cm"""
...
```math
\begin{align*}
    \tilde{F}^{(4)} & = \tilde{F}_6 + \tilde{F}_7 \\
    \tilde{F}^{(5)} & = \tilde{F}_8 + \tilde{F}_9 \\
    \tilde{F}^{(6)} & = \tilde{F}_{10} + \tilde{F}_{11} \\
\end{align*}
```

"""

# ╔═╡ 00453a5a-7fdc-4a31-9e1e-5cca66a7650f
begin
	fg7 = Figure()
	...
	axfg7 = Axis(fg7[1, 1],
	    xlabel = "t",
	    ylabel = "Acceleration (g)",
	    title = "Walking Time Series: Next Three Groups (Zoomed)",
	)
	
	#lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,1), label=CairoMakie.L"\tilde{F}^{(1)}")
	#lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,2:3), label=CairoMakie.L"\tilde{F}^{(2)}")
	#lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,4:5), label=CairoMakie.L"\tilde{F}^{(3)}")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,6:7), label=CairoMakie.L"\tilde{F}^{(4)}")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,8:9) .+ 0.25, label=CairoMakie.L"\tilde{F}^{(5)}")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,10:11) .+ 0.5, label=CairoMakie.L"\tilde{F}^{(6)}")
	...
	axislegend(axfg7,framevisible = false,position = :ct,orientation = :horizontal)
	
	fg7
end

# ╔═╡ fd8a45ff-f373-4235-8d51-d5e449f833b3
cm"""
...
# ╔═╡ 6e4266d7-d7a1-4148-84cd-1d2666e6c018
begin
	fg8 = Figure()
...
# ╔═╡ bbe49a10-7d33-4637-aaf0-5b19bf017a21
cm"- Combining the first 11 elementary components together, and plotting the remaining 339:"

# ╔═╡ a412a26d-1916-478a-a96f-e4f6a419c506
begin
	fg9 = Figure()
	...
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,1:11),label="First 11 Components")
	lines!(index_1[rng], SSAJL.reconstruct(accel_ssa,12:350),label="Remaining 339 Components")
	lines!(index_1[rng], accel_1[rng], label="Original time-series")
	
	xlims!(16,20)
	ylims!(-0.5, 2.5)
	
	axislegend(axfg9,framevisible = false,position = :ct,orientation = :horizontal)
	
	fg9
end

# ╔═╡ dc4a8a45-c751-4d75-9679-c0dfee8bbc89
cm"""
...
- Let's pretend we were given a dataset consisting of unlabelled walking accelerometer time series, some of them from the same person, 
perhaps, and tasked with determining the number of individuals in the dataset.
...
> This application of SSA has been inspired by the papers [Lamar-Leon et al. (2017)](https://arxiv.org/pdf/1707.06982.pdf) and, of course, the source of this dataset is [Malekzadeh et al. (2018)](https://arxiv.org/abs/1802.07802).
"""
...
function extract_series(subject, walk)
    # Mapping walk integers to folder names
    walk_dict = Dict(1 => "wlk_8", 2 => "wlk_15")
    
    # Constructing the file path
    file_path = "A_DeviceMotion_data/$(walk_dict[walk])/sub_$(subject).csv"
    
    # Read the CSV into a DataFrame
    df_walk = CSV.read(file_path, DataFrame)
    
    # Select columns and calculate the magnitude (L2 norm)
    # In Julia, . denotes vectorized operations
    cols = ["userAcceleration.x", "userAcceleration.y", "userAcceleration.z"]
    accel = sqrt.(sum.(eachrow(df_walk[:, cols] .^ 2)))
    
    # Convert acceleration values to z-scores
    accel = (accel .- mean(accel)) ./ std(accel)
    
    # Create a time index (assuming 50Hz based on your Python code)
    # Julia's index is 1-based, so we create a range for the time axis
    time_index = (0:length(accel)-1) ./ 50
    
    # Filter by time (10 to 25 seconds)
    start_time, end_time = 10.0, 25.0
    mask = (time_index .>= start_time) .& (time_index .<= end_time)
    accel_subset = accel[mask]
    
    # Perform SSA (Assuming you have an SSA function defined elsewhere)
    window = 350
    return SSAJL.SSA(accel_subset, window)
end
...
begin
	...
	for subject in subjects
	    ...
	    # In Julia, commas create a Tuple automatically
	    subject_ssa[subject] = (extract_series(subject, 1), extract_series(subject, 2))
	end
end

# ╔═╡ 59b8270c-0e45-4c04-8a8f-1740206a0807
cm"- Plot all of the extracted 'signatures', using the first four eigentriples of the SSA decomposition:"

# ╔═╡ 41bbe8af-86f6-4bbf-9bec-e2827b671327
begin
	# Setup data
	genders = vcat(fill("F", 2), fill("M", 3))
	
	# Loop through subjects and genders
	for (subject, gender) in zip(subjects, genders)
	    # Extract SSA objects
	    ssa_1, ssa_2 = subject_ssa[subject]
	    
	    # Reconstruct data (assuming reconstruct returns a plottable signal)
	    # Using 1:4 for Julia's 1-based indexing equivalent to Python's 0:4
	    sig1 = SSAJL.reconstruct(ssa_1, 1:4)
	    sig2 = SSAJL.reconstruct(ssa_2, 1:4)
	
	    time_index = 10. .+ (0:length(sig1)-1) ./ 50
	    
	    # Create the figure and axis
	    fig = Figure(size = (700, 600))
	    ax = Axis(fig[1, 1],
	        title = "Participant #$subject Walk 'Signature' ($gender)",
	        xlabel = L"t \text{ (s)}",
	        ylabel = "Z-Score",
	        limits = (16, 20, -1.5, 1.5)
	    )
	
	    # Plot lines
	    lines!(ax, time_index, sig1, color = :red, label = "Walk #1")
	    lines!(ax, time_index, sig2, color = :blue, label = "Walk #2")
...
# ╔═╡ 72a8fe6d-1d5a-465f-846f-0e58e28968d5
cm"""
### Final Words
***
...
- [Lamar-Leon et al. (2017) - "Persistent-homology-based gait recognition"](https://arxiv.org/pdf/1707.06982)
- [Malekzadeh et al. (2018) - "Protecting Sensory Data against Sensitive Inferences"](https://arxiv.org/abs/1802.07802)
"""

# ╔═╡ f0c28b9a-b044-421c-9939-4abbbfb2d6d3
cm"""
### Credits
***

*This notebook is obtained and further elaborated from: [https://www.kaggle.com/code/jdarcy/introducing-ssa-for-time-series-decomposition](https://www.kaggle.com/code/jdarcy/introducing-ssa-for-time-series-decomposition) with data from [https://github.com/mmalekzadeh/motion-sense](https://github.com/mmalekzadeh/motion-sens)*.
"""
...
manifest_format = "2.0"
project_hash = "fd173123320f35c0c3630e86b1f525f2e3e81bd2"
