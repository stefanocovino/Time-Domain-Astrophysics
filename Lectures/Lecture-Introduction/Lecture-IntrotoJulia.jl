...
- This notebook will teach you some of the basic programming routines in Julia. We will assume basic familiarity with programming, such as for-loops, if-else statements and function definitions.
"""
...
md"""
- Julia also has a `Symbol` type. A `Symbol` type is characterized by a `:` in front of a word. For example:
"""
...
- A few things to be aware of when working in Pluto notebooks:
  ...
  - The output of a cell is shown above and not below the cell itself. One might need some time to get used to it.
"""
...
- Reference: [Makie.jl](https://docs.makie.org/stable/)

Makie.jl provides a common interface to call various other visualization backends (Cairo, GL, WGL, etc.). In other words, commands like `lines(..)` will be translated into the appropriate command for the backend. So, it does not re-invent the wheel but rather gives you access to the already enormous field of visualization tools.

Below are a few examples of the most common visualization commands you'll see throughout the course. 
"""
...
begin
    fig1 = Figure()
    ax1 = Axis(fig1[1, 1], xlabel="x", ylabel="p(x)")
    
    # 2. Plot the line onto the axis
    lines!(ax1, xx, pdf.(px, xx), 
        label="pdf", 
        color=:red, 
        linewidth=5, 
        linestyle=:dash
...
begin
    fig2 = Figure()
    ax2 = Axis(fig2[1, 1], xlabel="X_1", ylabel="X_2")
    
    # 2. Plot the scatter points onto the axis
    scatter!(ax2, X[:,1], X[:,2], 
        markersize=10, 
        marker=:circle # Note: :circle is the default in Makie anyway
    )
...
md"""
- It is of course possible to add more plots to the same figure.
"""

# ╔═╡ 909a83f1-507c-4d8f-855e-f1a45d3aa927
begin
    fig3 = Figure()
    ax3 = Axis(fig3[1, 1], xlabel="x", ylabel="p(x)")
    
    # First line (equivalent to your initial plot)
    lines!(ax3, xx, pdf.(Normal(0., 1.), xx), 
        label="N(0,1)", 
        color=:red, 
        linewidth=5, 
        linestyle=:dash
    ...
    # Second line (equivalent to your plot!)
    lines!(ax3, xx, pdf.(Normal(1., 0.5), xx), 
        label="N(1,0.5)", 
        color=:blue, 
        linestyle=:solid
    )
...
# ╔═╡ 0bea10ce-93e9-4865-b086-53bdf490c835
md"""
## Reference & Material
***
...
# ╔═╡ 35efdefa-211f-451a-970f-eae523999abc
md"""
### Credits
***

This notebook contains material obtained by [https://bmlip.github.io/course/probprog/Intro%20to%20Julia.html](https://bmlip.github.io/course/probprog/Intro%20to%20Julia.html).
"""
...
# ╔═╡ 3c931897-f58a-4532-8331-c0c131e8ff0f
html"""
<table>
  <tr>
	<td></td>
...
# ╔═╡ be2b9383-10fc-498b-a644-1d80676b81d0
md"""
**Copyright**
