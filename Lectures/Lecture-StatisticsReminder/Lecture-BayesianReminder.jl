...
# ╔═╡ ac8be882-1ffb-4176-bad6-3b30764d4b2b
WidthOverDocs()  

# ╔═╡ 051694a5-8c25-4155-a9df-b3ce18e18939
TableOfContents()
...
# ╔═╡ fdb52aa2-37c8-4ef8-b4ed-a43e0cacd5f1
md"""
Prevalence: $( @bind prev NumberField(0:0.01:1,default=0.1) ) 

Sensitivity: $( @bind sens NumberField(0:0.01:1,default=0.95) )

Specificity: $( @bind spec NumberField(0:0.01:1,default=0.85) )
"""

# ╔═╡ b0b148a1-2ae5-4681-8757-441e469173e3
f(prev,sens,spec) = sens * prev / (sens*prev + (1-spec)*(1-prev));

# ╔═╡ cae46801-d57d-4315-b03f-2d8d660978b1
Markdown.parse("""

##### Test: $(latexify(f(prev,sens,spec),fmt="%.2f"))
...
- In a frequentist scenario hypothesis testing is often carried out comparing the [likelihood ratio](https://en.wikipedia.org/wiki/Likelihood-ratio_test) of different models or the residuals by, e.g. a [F-test](https://en.wikipedia.org/wiki/F-test).

    - These tests are either related to the assumptions that data are drawn from a Normal parental distributions or do not take into account the number of parameters of the models. There are more limitations we don't discuss here (see, e.g., [Protassov et al. 2002](https://ui.adsabs.harvard.edu/abs/2002ApJ...571..545P/abstract)).
...
- Assuming to have two (or more) competing models, computing the *evidence*, the denominator of the previous expression, we can evaluate the **odds ratio**:

```math
O_{21} \equiv \frac{p(M_2|D,I)}{p(M_1|D,I} = \frac{p(M_2|I)}{p(M_1|I)} \frac{p(D|M_2)}{p(D|M_1)} = \frac{p(M_2|I)}{p(M_1|I)} B_{ij}
```

- The first factor of the odds ratio is the prior odd ratio, and the second factor is called the **Bayes factor**.
...
```math
p(D|M_1) \sim p(\hat{\theta}|M_1) \int \mathcal{L}(\theta) d\theta
```

- Then, we write the integral of the likelihood as the product of its maximum times a characteristi width (e.g., a full width at haf maximum, etc.):

```math
p(D|M_1) \sim p(\hat{\theta}|M_1) \mathcal{L}(\hat{\theta}) \delta\theta
```

- And finally, for clarity, we write $p(\hat{\theta}|M_1)$ as the reciprocal of a characteristic width of the prior, $\Delta \theta$. Thus:

```math
p(D|M_1) \sim \mathcal{L}(\hat{\theta}) \frac{\delta\theta}{\Delta \theta}
```

- Since $M_0$ has no free parameters $p(D|M_0) = \mathcal{L}(\theta_0)$, and the Bayes factor turns out to be:

```math
B_{10} \sim \frac{\mathcal{L}(\hat{\theta})}{\mathcal{L}(\theta_0)} \frac{\delta\theta}{\Delta \theta}
```

- The likelihood ratio can never be less than unity, since $\theta_0$ is within the possible range of $\theta$, and the second factor penalizes the likelyhood ratio for the "wasted" parameter space that gets ruled out by the data ($\delta\theta$ is narrower than $\Delta\theta$).

> In any case, a full discussion of hypothesis testing in a Bayesian scenario is a very interesting topic, but definitely beyond the purpose of the corse. Nevertheless, during the course, we will often mention this matter. 


- There are simpler alternatives, although with further assumptions and limitations. 
    - However, they require to compute only the maximum likelihood, and not the whole posterior distribution of paramaters.
...
- where $L^0[M]$ is the maximum likelihood for model $M$, $k$ the number of model parameters and $N$ is the number of data points.
...
```math
AIC \equiv -2 \log [L^0(M)] + 2k + \frac{2k(k+1)}{N-k-1}
```
...
# ╔═╡ 59006f2e-eb6b-45c8-9b2a-d5eeefa05978
...
# ╔═╡ da3bd540-6e87-462b-8a93-e0c0ff98e44b
Markdown.parse("""

##### ML for model 1 = $(latexify(χ21(prs1),fmt="%.2f")) and model 2 = $(latexify(χ22(prs2),fmt="%.2f"))

##### BIC for model 1 =  $(latexify(bc1,fmt="%.2f")) and model 2 = $(latexify(bc2,fmt="%.2f"))
...
- If we use [decibel](https://en.wikipedia.org/wiki/Decibel) units, the support for the simpler model turns out to be $\sim 18$ dB.
"""
...
# ╔═╡ 0fafb15b-8516-47e8-8016-bd4bc05d87a6
...
# ╔═╡ 2f1c4650-d591-4988-b1c7-d83f297bffc5
Markdown.parse("""

##### Parallax weighted mean: $(latexify(μp,fmt="%.3f")) mas and error on the mean: $(latexify(σp,fmt="%.3f")) mas
...
- What we have written so far holds for a single measurement. However, for the M67 distance, we work with a set of $n$ measurements for stars that we might now think are located at approximately the same distance $d_{\rm cluster}$ with measured parallaxes $y = {y_1, y_2, . . . , y_n}$, that we consider independent given $d_{\rm cluster}$.
...
# ╔═╡ 64f68823-25c4-4a21-a65b-b5e1a7194103
cm"""
- The posterior PDF of the cluster distance is nicely consistent with the inverse of the simple average of the star's cluster parallaxes (dot-dashed line). This happens since data are abundant and of high quality and sepersede the prior information making them of low relevance.

> This simple example also clarifies why, even if a Bayesian approach is far superior, frequentist results can often be adequate.
"""
...
# ╔═╡ 21528a8f-f833-49b1-afab-751307c220aa
md"""
- The posterior PDF of the cluster distance is nicely consistent with the inverse of the simple average of the star's cluster parallaxes (dot-dashed line). This happens since data are abundant and of high quality and supersede prior information making them of low relevance.

> This simple example also clarifies why, even if a frequentist result is often enough, the Bayesian approach is far superior providing the ful PDF of the parameter(s) of interest given the data.
"""
...
# ╔═╡ 06f53c4f-6703-4ef3-ae2e-110f80139fa6
begin
	...
	figch = Figure(size=(1000, 800))

	for (i, param) in enumerate(params)
	    axch = Axis(figch[i, 1]; ylabel=string(param))
	    for ch in 1:n_chains
	        values = chain[:, param, ch]
	        lines!(axch, 1:n_samples, values; label=string(ch))
	    end
	
	    hideydecorations!(axch; label=false)
	    if i < length(params)
	        hidexdecorations!(axch; grid=false)
	    else
	        axch.xlabel = "Iteration"
	    end
	end

	for (i, param) in enumerate(params)
    	axch2 = Axis(figch[i, 2]; ylabel=string(param))
    	for ch in 1:n_chains
        	values = chain[:, param, ch]
        	CairoMakie.density!(axch2, values; label=string(ch), strokearound=false, strokewidth=2,strokecolor=Cycled(ch),color=:transparent)
    	end

    	axch2.xlabel = "Parameter estimate"
	end
...
# ╔═╡ 01925a5f-4d1e-4859-9f80-de412c38d954
cm"""
- Checking chain convergence is very important. 

- The ``\hat{r}`` parameter, or Potential Scale Reduction Factor (PSRF), is a MCMC convergence diagnostic that compares between-chain variance to within-chain variance for multiple chains. 

-  It measures if chains have mixed well; a value of ``\hat{r} \approx 1`` indicates convergence.

- It has been introduced in [Gelman & Rubin (1992)](https://sites.stat.columbia.edu/gelman/research/published/itsim.pdf).
"""

# ╔═╡ 00acc7b5-d951-4316-8e74-8441c67cfd6c
rhat(chain)

# ╔═╡ ec2e1acc-389e-40a9-bac0-33a81a867053
md"""
- The chains look fine.
- Let's produce a *corner plot*:
"""
...
# ╔═╡ c67a823e-3fb7-45ef-a8a1-1d35fda62190
md"- And let's plot a random selection of solutions from the posterior:"
...
# ╔═╡ 21ad0658-8982-4c37-a6ac-0b58d502ca21
md"Notebook v1.0.3 - 14 Sept 2026"
...
manifest_format = "2.1"
project_hash = "0697a5723903f85054e0be33a6428728480812ea"
